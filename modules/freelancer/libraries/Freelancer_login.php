...
class Freelancer_login {  
	...
	* @param string password dari input form*/
	...
	$query = $this->CI->db->get_where('freelancer',array('username'=>$username,'password' => md5($password)));
	if($query->num_rows() == 1) {
	//ambil data user berdasar username
	$row  		= $this->CI->db->query('SELECT id_freelancer FROM freelancer where username = "'.$username.'"');
	$admin      = $row->row();
	...
	redirect(base_url('freelancer'));}else{
